...
export class UserService {
    ...
    changePassword(id: string,oldPassword: string, newPassword: string, confirmPassword: string): Observable<boolean> {
        return this.http.put<boolean>(`${this.apiUrl}/changePassword/${id}`, { oldPassword, newPassword, confirmPassword }, {
            headers: getAuthHeaders()
            });
    }
  ...
  sendResetPasswordEmail(email: string): Observable<ApiResponse> {
    return this.http.post<ApiResponse>(`${this.apiUrl}/resetPassword-send-mail`, { email });
  }

  verifyResetToken(token: string): Observable<ApiResponse> {
    return this.http.get<ApiResponse>(`${this.apiUrl}/reset-password/verify?token=${token}`);
  }

  resetPassword(newPassword: string, token: string): Observable<ApiResponse> {
    return this.http.post<ApiResponse>(`${this.apiUrl}/reset-password/new`, { newPassword, token });
  }
