...
export class SupplierService {
  ...
  updateSupplier({
    ...
    return this.http.put<Supplier>(
      `${this.apiUrl}/updateSupplier/${_id}`,
      supplier,
      {
        headers: getAuthHeaders(),
      }
  ...
  addProductToSupplier(supplierId: string, product: any): Observable<any> {
    return this.http.post<any>(
      `${this.apiUrl}/addProductToSupplier`,
      { supplierId: supplierId, products: [product] },
      {
        headers: getAuthHeaders(),
      }
    );
  }
  getProductOfSupplier(_id: string): Observable<Supplier> {
    return this.http.get<Supplier>(`${this.apiUrl}/getSupplierProduct/${_id}`, {
      headers: getAuthHeaders(),
    });
  }
}
