...
//     const barcodeToUse = this.barcode;
...
export class BillComponent implements OnInit, AfterViewInit {
  ...
  filterBarcodes(event: any): void {
    ...
    // Update the barcode field with the current input value
    this.barcode = event.query;
  }
  ...
  addProductToBill(): void {
    ...
    // Get actual input value from autocomplete if there's no selected item
    let barcodeToUse = this.barcode;
    if (!barcodeToUse && this.itemInput) {
      const inputElement = this.itemInput.el.nativeElement.querySelector('input');

      if (inputElement) {
        barcodeToUse = inputElement.value;
      }
    }
    ...
    // Check if the input is a valid barcode
    const isValidBarcode = this.barcodes.some(
      (item) => item.barcode.toString() === barcodeToUse
    );
